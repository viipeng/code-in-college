...
public class Reader_change extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String sql = "update Reader set "+attribute+" = '"+ value + "' where rename = '" + rename +"';";
		try {
			a=aq.change(sql);
			if(a==1)
			out.println("<script language='javascript'>alert('更新成功！');window.location.href='Reader/Reader_change.jsp';</script>");
			else
			request.getRequestDispatcher("/Reader/Reader_change.jsp").forward(request, response);
