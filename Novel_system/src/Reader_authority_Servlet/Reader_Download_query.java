...
public class Reader_Download_query extends HttpServlet {
...
//		PrintWriter out = response.getWriter();
		...
		try {
			...
			if(Download.next()){
                ...
                RequestDispatcher dispatcher = request.getRequestDispatcher("/Reader_authority/Download_query1.jsp");
                dispatcher .forward(request, response);
				
			}
			else{
	             JOptionPane.showMessageDialog(null, "²éÑ¯Ê§°Ü", "³ö´íÀ²", JOptionPane.ERROR_MESSAGE);
				 RequestDispatcher dispatcher = request.getRequestDispatcher("/Reader_authority/Download_query.jsp");
	             dispatcher .forward(request, response);
			}
