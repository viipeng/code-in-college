	...
	<table class="table table-border table-bordered table-bg table-sort">
		...
		<c:forEach items="${requestList }" var="rl">
			<tr class="text-c">
				<td>${rl.aid }</td>
				<c:choose>
					<c:when test="${rl.state==1 }">
						<td>同意</td>
					</c:when>
					<c:when test="${rl.state==2 }">
						<td>拒绝</td>
					</c:when>
					<c:when test="${rl.state==0 }">
						<td>未处理</td>
					</c:when>
				</c:choose>
				<td>${rl.id }</td>
				...
				<c:choose>
					<c:when test="${rl.state==0 }">
				<td class="td-manage"> 
					<a title="同意" href="${pageContext.request.contextPath }/bossInfo/permit?aid=${rl.aid }" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e1;</i></a> 
					<a title="不同意" href="${pageContext.request.contextPath }/bossInfo/refuse?aid=${rl.aid }" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6dd;</i></a>
				</td>
					</c:when>
					<c:when test="${rl.state==1 }">
