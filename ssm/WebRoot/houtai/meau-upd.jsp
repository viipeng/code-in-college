...
<meta name="description" content="修改菜单信息">
</head>
<body>
<article class="page-container"> 
	<form action="${pageContext.request.contextPath }/meauInfo/updateMeau" method="post" class="form form-horizontal" onsubmit="return check()"> <!-- id="form-member-add" -->
		<c:forEach items="${meauList }" var="meau">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>菜单号：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${meau.mid }" placeholder="" id="mid" name="mid" readonly="true" >
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>菜单名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${meau.mname }" placeholder="" id="mname" name="mname">
			</div>
		</div>
		<c:choose>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${meau.rid }" placeholder="" id="rid" name="rid">
			</div>
		</div>
			</c:when>
			<c:when test="${sessionScope.rid!=null }">
				<input type="hidden" value="${meau.rid }" id="rid" name="rid" >
			</c:when>
		</c:choose>
		<div class="row cl">
...
<script type="text/javascript">
function check(){
	if(isNaN(document.getElementById('rid').value)){
		alert('RID只能输入数字');
        return false;
    }
	else{
