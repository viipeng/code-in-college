...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 
	...
	<span class="c-gray en">&gt;</span> 菜品列表 
		<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" >
			<i class="Hui-iconfont">&#xe68f;</i></a>
</nav>
<div class="page-container">
	<div class="text-c"> 
		<form action="${pageContext.request.contextPath }/foodInfo/selectByFid" method="post">
			<input type="text" class="input-text" style="width:250px" placeholder="输入菜品号" id="" name="fid1">
				<button type="submit" class="btn btn-success radius" id="" name="">
					<i class="Hui-iconfont">&#xe665;</i> 搜菜品</button>
		</form>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
		<a href="javascript:;" onclick="delManyData()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
		<a href="../foodInfo/jumpToAdd"  class="btn btn-primary radius">
			<i class="Hui-iconfont">&#xe600;</i> 添加菜品</a>
		<a href="${pageContext.request.contextPath }/foodInfo/selectAll" class="btn btn-success radius">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="${fi.fid }" name="check"></td>
				<td>${fi.fid}</td>
				<td>${fi.fname}</td>
				<td>${fi.fphoto}</td>
				<td>${fi.fprice}</td>
				<td>${fi.fdescription}</td>
				<td>${fi.fsales_volume}</td>
				<td>${fi.great}</td>
				<td>${fi.mid}</td>
				<td class="td-manage"> 
					<a title="修改" href="${pageContext.request.contextPath }/foodInfo/jumpToUpdate?fid=${fi.fid}" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="${pageContext.request.contextPath }/foodInfo/deleteFood?fid=${fi.fid}" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i></a>
				</td>
			</tr>
		    </c:forEach>
...
/* 			alert(cks[i].value); */
	...
	/* alert(str); */
	location.href="${pageContext.request.contextPath }/foodInfo/deleteManyFood?str="+str;
}
</script>  
</body>
</html>