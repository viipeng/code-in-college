...
<div class="page-container">
	<div class="text-c"> 
		<form action="${pageContext.request.contextPath }/orderInfo/selectByOid" method="post">
			<input type="text" class="input-text" style="width:250px" placeholder="输入订单ID" id="" name="oid1">
				<button type="submit" class="btn btn-success radius" id="" name="">
					<i class="Hui-iconfont">&#xe665;</i> 搜订单</button>
		</form>
	</div>
	...
	<c:choose>
		...
		<a href="../orderInfo/jumpToAdd"  class="btn btn-primary radius">
			<i class="Hui-iconfont">&#xe600;</i> 添加订单</a>
		</c:when>
	</c:choose>
		<a href="${pageContext.request.contextPath }/orderInfo/selectAll" class="btn btn-success radius">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<tr class="text-c">
				<td><input type="checkbox" value="${order.oid }" name="check"></td>
				<td>${order.oid}</td>
				<td>${order.rname}</td>
				<td>${order.rphoto}</td>
				<td>${order.all_food}</td>
				...
				<c:choose>
					<c:when test="${sessionScope.rid==null }">
					<a title="修改" href="${pageContext.request.contextPath }/orderInfo/jumpToUpdate?oid=${order.oid}" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="${pageContext.request.contextPath }/orderInfo/deleteOrder?oid=${order.oid}" class="ml-5" style="text-decoration:none">
						<i class="Hui-iconfont">&#xe6e2;</i></a>
					</c:when>
				</c:choose>
				</td>
	...
	if(${sessionScope.rid==null }){
		$('.table-sort').dataTable({
			...
			"aoColumnDefs": [
			  ...
			  {"orderable":false,"aTargets":[0,2,3,4,11]}// 制定列不参与排序
			]
		});
	}
	else{
		$('.table-sort').dataTable({
			...
			"aoColumnDefs": [
			  //{"bVisible": false, "aTargets": [ 3 ]} //控制列的隐藏显示
			  {"orderable":false,"aTargets":[0,2,3,4,9]}// 制定列不参与排序
			]
		});
	}
...
/* 			alert(cks[i].value); */
	...
	/* alert(str); */
	location.href="${pageContext.request.contextPath }/orderInfo/deleteManyOrder?str="+str;
}
</script>  
</body>
</html>
